...
with open("clinicas_dentarias_portugal_leads.json", "r", encoding="utf-8") as f:
    pt_leads = json.load(f)

msg_pt = "Olá, tudo bem? O meu nome é Leonardo, gostaria de falar consigo a respeito de uma proposta."
for l in pt_leads:
    l["country"] = "PT"
    l["country_label"] = "Portugal"
    l["flag"] = "🇵🇹"
    l["lang"] = "pt-PT"
    l["mensagem_personalizada"] = msg_pt
    l["wa_link_com_mensagem"] = f"{l['wa_link']}?text={quote(msg_pt)}"

all_leads = br_leads + pt_leads

print(f"Total BR leads: {len(br_leads)} | Total PT leads: {len(pt_leads)} | Total All: {len(all_leads)}")

# Save updated JSONs
with open("clinicas_dentarias_portugal_leads.json", "w", encoding="utf-8") as f:
    json.dump(pt_leads, f, ensure_ascii=False, indent=2)
...
<body>
  ...
  <div class="container">
    <header>
      ...
      <h1 id="header-title">Painel de Leads • Clínicas Odontológicas</h1>
    ...
    <div class="country-bar">
      <button class="country-btn active" id="btn-country-all" onclick="setCountry('ALL')">
        <span>🌐 Todos os Países</span>
        <span class="badge-count" id="count-all">{len(all_leads)}</span>
      </button>
      <button class="country-btn" id="btn-country-br" onclick="setCountry('BR')">
        <span>🇧🇷 Brasil</span>
    ...
    <div class="lang-notice" id="lang-indicator">
      <span>🌐 <strong>Idioma Adaptativo Ativo:</strong> Mensagens em Português de Portugal (PT-PT) para leads de Portugal e Português do Brasil (PT-BR) para leads do Brasil.</span>
    </div>
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card">
        ...
        <span class="stat-value gold" id="total-available">{len(all_leads)}</span>
      </div>
    </div>
  ...
  <script>
    ...
    let currentCountry = 'ALL'; // 'ALL' | 'BR' | 'PT'
    ...
    function setCountry(country) {{
      ...
      // Update UI texts to match regional vocabulary
      ...
      if (country === 'PT') {{
        ...
        langIndicator.innerHTML = '🇵🇹 <strong>Idioma: Português de Portugal (PT-PT)</strong> — Mensagens e termos adaptados ("gostaria de falar consigo", "telemóvel", "freguesia").';
      }} else if (country === 'BR') {{
        ...
        langIndicator.innerHTML = '🇧🇷 <strong>Idioma: Português do Brasil (PT-BR)</strong> — Mensagens e termos adaptados ("quero falar a respeito", "celular", "bairro").';
      }} else {{
        ...
        langIndicator.innerHTML = '🌐 <strong>Idioma Adaptativo Ativo:</strong> Mensagens em Português de Portugal (PT-PT) para leads de Portugal e Português do Brasil (PT-BR) para leads do Brasil.';
      }}
    ...
    function renderLeads() {{
      ...
      list.forEach((lead, index) => {{
        ...
        const langBadge = isPt ? '🇵🇹 PT-PT' : '🇧🇷 PT-BR';
        ...
        card.innerHTML = `
          <div class="lead-info">
            <div class="lead-header">
              ...
              <span class="rating-badge">★ ${{lead.rating}} (${{lead.reviews}} ${{isPt ? 'avaliações' : 'avaliações'}})</span>
            </div>
            ...
            <div class="msg-box-header">
              ...
              <span style="color: var(--accent);">${{langBadge}}</span>
            </div>
...
print("disparador_whatsapp.html com Português de Portugal e Brasil atualizado!")
